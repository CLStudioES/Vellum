...
function App() {
  ...
  // Selection state: "filename::key" as composite identifier
  ...
  async function handleLoadProjects() {
    ...
    try {
      const result: ProjectSummary[] = await invoke("list_projects", { passphrase });
      setProjects(result);
  ...
  function getFileStats(file: EnvFile) {
    ...
    return {
      ...
      selected: data.filter((e) => selected.has(compositeKey(file.filename, e.key))).length,
    };
  }
              ...
              <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-5">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      setAssignMode("existing");
                      if (passphrase) handleLoadProjects();
                    }}
                    className={`px-4 py-2 rounded-lg font-mono text-xs transition-colors ${
                      assignMode === "existing"
                        ? "bg-indigo-600 text-white"
                ...
                {assignMode === "existing" && (
                  <div className="space-y-2">
                    {projects.length === 0 && passphrase && (
                      <button
                        onClick={handleLoadProjects}
                        className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 font-mono text-xs text-slate-400 hover:text-slate-200 hover:border-slate-600 transition-colors"
                      >
...
function Badge({ text, color }: { text: string; color: string }) {
  return (
    <span
      className={`text-[10px] bg-${color}-900/50 text-${color}-400 px-1.5 py-0.5 rounded shrink-0`}
    >
      {text}
    </span>
  );
